...
os.makedirs('/tmp/blit_pdf_thumbnails', exist_ok=True)

def prepare_thumb(path):
    if not os.path.exists(path):
        return None
    thumb_name = os.path.splitext(os.path.basename(path))[0] + '_thumb.jpg'
    out_path = os.path.join('/tmp/blit_pdf_thumbnails', thumb_name)
    with Image.open(path) as img:
        # Resize to thumbnail dimension (300x300 at high quality)
...
print(f"Optimized visual PDF catalogue generated: {size_mb:.2f} MB")
